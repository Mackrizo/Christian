...
define ( 'ACTION_READ_ALL', 'read_all_with_select_star' );
define ( 'ACTION_READ_ALL_WITH_VARIOUS_COLUMN', 'read_all_with_various_column' );
...
class Factory {
	...
	public function read_all($object) {
		$this->_object = $object;
		$query = $this->_generate_query ( ACTION_READ_ALL, null );
		return $this->_execute_query ( $query, ACTION_READ_ALL );
	}
	...
	private function _generate_query($action, $criteria) {
		...
		foreach ( $properties as $key => $value ) {
			...
			if ($key != 'id') {
				// lister les valeurs
				$values .= ($key != end ( $properties_names )) ? "'" . $value . "', " : "'" . $value . "'";

				// lister les assignations
				$settings .= ($key != end ( $properties_names )) ? $key . "='" . $value . "', " : $key . "='" . $value . "'";

				// lister les éléments de la clause WHERE
				$where .= ($key != end ( $properties_names )) ? $key . " LIKE '%" . $criteria . "%' OR " : $key . " LIKE '%" . $criteria . "%' ";
			}
		}
	...
	private function _execute_query($query, $action) {
		...
		if ($succes = mysqli_query ( $con, $query )) {
			...
			if ($action == ACTION_READ_ALL)
				$result = $this->_get_result ( $succes, ACTION_READ_ALL );
	...
	private function _get_result($result, $action) {
		...
		if ($action == ACTION_READ || $action == ACTION_READ_ALL)
			// retourner l'objet
			return $object;
		else
			return $list;
